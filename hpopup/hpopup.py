...
============
 ...
 The :class:`HPopup` is the extension for the :class:`~kivy.uix.popup.Popup`
...
.. warning::
    * Normalization is applied once (when using the :meth:`HPopup.open`)
...
This example creates a simple popup with specified minimum size::
    ...
    popup = HPopup(size_hint=(.4, .3), min_width=400, min_height=300)
    popup.open()
...
be normalized to the size of the app window)::
    ...
    popup = HPopup(size=(1000, 1000), size_hint=(None, None),
                   fit_to_window=True)
    popup.open()
"""
...
class HPopup(Popup):
    """HPopup class. See module documentation for more information.
    """
